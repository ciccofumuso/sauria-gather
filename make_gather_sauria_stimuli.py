...
def patch_mem_with_identity_csr(mem: list[int], comp_base: int, idx_base: int, n_values: int) -> None:
    # One CSR row with n_values nonzeros, plus trailing zero sentinel for current gather FSM.
...
def build_gather_rows(dense_base: int, comp_base: int, idx_base: int, n_values: int,
                      n_dense_cols: int, debug_reads: int) -> list[list[int]]:
    comp_count = 3
    idx_count = n_values
    rows = [
        ...
        stim_write(gaddr(REG_TOTAL_LEN_COMP_IDX), nbeats(comp_count * IDX_BYTES)),
        stim_write(gaddr(REG_AR_ADDR_IDX), idx_base),
        stim_write(gaddr(REG_TOTAL_LEN_IDX), nbeats(idx_count * IDX_BYTES)),
...
def main() -> None:
    ...
    for mem in (init, gold):
        patch_mem_with_identity_csr(mem, args.comp_base, args.idx_base, args.n_values)
